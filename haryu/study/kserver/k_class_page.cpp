...
void Server::changeEvents(VEC<struct kevent>& changeList_, uintptr_t ident, int16_t filter, uint16_t flags, uint16_t fflags, intptr_t data, void* udata) {
...
Server::Server(int portNumber_, char **port_) : msg_("") {
	...
	for (int i = 0; i + 1 < portNumber_; i++) {
		...
		if (serverSocket_[i] == -1) { exitWithPerror("socket() error\n" + String(strerror(errno))); }
...
void Server::myBind() {
	...
	for (int i = i; i < portnum_; i++) {
		ret = bind(serverSocket_[i], reinterpret_cast<struct sockaddr *>(&serverAddr_[i]), sizeof(serverAddr_[i]));
...
int	Server::mySetSockopt(int val , int fd) {
	...
	if (val == SO_REUSEADDR) {
		...
		for (int i = 0; i < portnum_; i++) {
			ret = setsockopt(serverSocket_[i], SOL_SOCKET, SO_REUSEADDR, &bf, sizeof(bf));
			if (ret == -1) { exitWithPerror("setsockopt(SO_REUSERADDR) error\n" + String(strerror(errno))); }
			COUT << "Socket(" << serverSocket_[i] << ")setting is succesed : SO_REUSEADDR" << CEND;
		}
		return (0);
	}
...
void	Server::turnOn() {
	kq_ = kqueue();
	if (kq_ == -1) { exitWithPerror("kq_ueue() error\n" + String	(strerror(errno))); }

	for (int i = 0; i < portnum_; i++) {
		changeEvents(changeList_, serverSocket_[i], EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, NULL);
	...
	while(true) {
		newEvents_ = kevent(kq_, &changeList_[0], changeList_.size(), eventList_, EVENT_MAX, nullptr);
		if (newEvents_ == -1) { exitWithPerror("kevent() error\n" + String(strerror(errno))); }
		...
		for (int i = 0; i < newEvents_; ++i) {
			...
			if (currEvent_->flags & EV_ERROR) {
				...
				if (IsServer(currEvent_->ident)) {
					...
					if (clientS_ocket == -1) { exitWithPerror("accept() error\n" + String(strerror(errno))); }
					COUT << "accept new client : " << clientS_ocket << CEND;
					fcntl(clientS_ocket, F_SETFL, O_NONBLOCK);

					/* add event for client socket - add read && write event */
					changeEvents(changeList_, clientS_ocket, EVFILT_READ, EV_ADD | EV_ENABLE | EV_EOF, 0, 0, NULL);
					changeEvents(changeList_, clientS_ocket, EVFILT_WRITE, EV_ADD | EV_ENABLE, 0, 0, NULL);
			...
			else if (currEvent_->filter == EVFILT_WRITE) {
				/* send Data to client*/
				std::map<int, String>::iterator it = clients_.find(currEvent_->ident);
				if (it != clients_.end()) {
					int n;
					n = send(currEvent_->ident, msg_.c_str(), strlen(msg_.c_str()), MSG_DONTWAIT);
